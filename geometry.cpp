...
template <> template <> Vec3<float>::Vec3(const Vec3<double> &v): x(float(v.x+.5)), y(float(v.y+.5)), z(float(v.z+.5)) {}
template <> template <> Vec3<double>::Vec3<>(const Vec3<float> &v) : x(v.x), y(v.y), z(v.z) {}
